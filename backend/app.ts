...
import { general_menu_buttons, TelegramCallbackData } from "./services/telegram/telegram";
...
bot.on('new_chat_members', async (msg: Message) => {
    ...
    if (newMembers) {
        for (const member of newMembers) {
            ...
            await bot.sendMessage(msg.chat.id, 'What do you want to do with the bot?', general_menu_buttons);
        }
    }
});
    ...
    if (message && data) {
        switch (data) {
            ...
            case TelegramCallbackData.REFER_FRIENDS:
                ...
                const shareLink = `https://t.me/ref${userId}`
                await bot.sendMessage(message.chat.id, `Here is your share link: ${shareLink}. Send it to your friend to earn token gift.`); // Gửi lại liên kết chia sẻ
                break;
            default:
